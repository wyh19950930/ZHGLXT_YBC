...
public class StorageUtils {
    ...
    /*
    获取SD目录路径，如果存在可移除介质，返回可移除介质根目录
     */
    public static String getSDPath(Context pContext){
        ArrayList<StorageBean> storageDatas = getStorageData(pContext);
        String innerPath="";
        if (storageDatas != null) {
            for (StorageBean s:storageDatas
                    ) {
                if (s.getRemovable()){
                    if (s.getMounted().equalsIgnoreCase("mounted")){
                        return s.getPath();
                    }
                }else {
                    innerPath= s.getPath();
