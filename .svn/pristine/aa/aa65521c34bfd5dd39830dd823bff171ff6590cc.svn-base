package com.jymj.zhglxt.widget.gxt;
...
import com.jymj.zhglxt.widget.gxt.bean.CanScrollBean;
import com.jymj.zhglxt.widget.gxt.bean.PointLeo;
import com.jymj.zhglxt.widget.gxt.bean.PointScroll;
import com.jymj.zhglxt.widget.gxt.bean.RectPoint;
import com.jymj.zhglxt.widget.gxt.bean.RelationBean;
...
//        image_theOne.setBorderWidth(DisplayUtil.dip2px(2));
...
//        leoParams_text.height = DisplayUtil.dip2px(20);
//        leoParams_text.leftMargin = DisplayUtil.dip2px(5000) / 2 + DisplayUtil.dip2px(17);
//        leoParams_text.topMargin = DisplayUtil.dip2px(5000) / 2 - DisplayUtil.dip2px(10);
//
//        textView.setTextColor(getResources().getColor(R.color.theme_text_black2d));
//        textView.setTextSize(DisplayUtil.dip2px(4));
...
//        this.clickList = sourceList;
        ...
        myAtmanList.get(0).setY_center(DisplayUtil.dip2px(5000) / 2);
        myAtmanList.get(0).setX_center(DisplayUtil.dip2px(5000) / 2);
        myAtmanList.get(0).setRectPoint(new RectPoint(DisplayUtil.dip2px(5000) / 2 - DisplayUtil.dip2px(23), DisplayUtil.dip2px(5000) / 2 - DisplayUtil.dip2px(23)
                , DisplayUtil.dip2px(5000) / 2 + DisplayUtil.dip2px(23), DisplayUtil.dip2px(5000) / 2 + DisplayUtil.dip2px(23)));
//        sourceList.remove(myselfAtman);
    ...
    public void addPoint() {
        ...
        if (objectMy >= HavePic) {
            ...
            image_theOne.setImageResource(R.drawable.transt_test);
        ...
        reParams.height = DisplayUtil.dip2px(objectMy);
        reParams.width = DisplayUtil.dip2px(objectMy);

        RelativeLayout.LayoutParams reParamsWave = (RelativeLayout.LayoutParams) waveView.getLayoutParams();
        reParamsWave.height = DisplayUtil.dip2px(objectMy + 45);
        reParamsWave.width = DisplayUtil.dip2px(objectMy + 45);
        ...
        if (checkClickIn(myselfAtman.getGroup())) {//判断我是不是在里面
            ...
            if (clickList != null) {//如果点击集合里不为空才加名字
                ...
                LayoutParams leoParams_text = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                leoParams_text.height = DisplayUtil.dip2px(20);
                leoParams_text.leftMargin = DisplayUtil.dip2px(5000) / 2 + DisplayUtil.dip2px(objectMy / 2);
                leoParams_text.topMargin = DisplayUtil.dip2px(5000) / 2 - DisplayUtil.dip2px(10);

                textView.setTextColor(getResources().getColor(R.color.black));//R.color.white
                textView.setTextSize(DisplayUtil.dip2px(4));
                textView.setGravity(Gravity.CENTER);
        ...
        for (int i = 0; i < oneAtmanList.size(); i++) {
            ...
            int trueX1 = X1 - DisplayUtil.dip2px(objectBig / 2);//减去自身控件的长度
            int trueY1 = Y1 - DisplayUtil.dip2px(objectBig / 2);//减去自身控件的长度


            //当前点的所在区域则是
            RectPoint leoPoint = new RectPoint(trueX1 - DisplayUtil.dip2px(objectBig / 2 + 13), trueY1 - DisplayUtil.dip2px(objectBig / 2 + 13),
                    trueX1 + DisplayUtil.dip2px(objectBig / 2 + 13), trueY1 + DisplayUtil.dip2px(objectBig / 2 + 13));
            ...
            if (checkHaveRect(leoPoint)) {//检查是否有重叠，true为有重叠。要增加半径的长度
                ...
                leoParams_bottom_yy.width = DisplayUtil.dip2px(objectBig);
                leoParams_bottom_yy.height = DisplayUtil.dip2px(objectBig);
                ...
                if (objectBig >= HavePic) {
                    ...
                    imageView_bottom_yy.setBorderWidth(DisplayUtil.dip2px(2));
                ...
                if (checkClickIn(itemBean.getGroup())) {//判断我是不是在里面
                    ...
                    if (clickList != null) {
                        ...
                        LayoutParams leoParams_text = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                        leoParams_text.height = DisplayUtil.dip2px(20);
                        leoParams_text.leftMargin = X1 + DisplayUtil.dip2px(objectBig / 2);
                        leoParams_text.topMargin = Y1 - DisplayUtil.dip2px(10);
                ...
                itemBean.setRectPoint(new RectPoint(trueX1 - DisplayUtil.dip2px(objectBig / 2), trueY1 - DisplayUtil.dip2px(objectBig / 2),
                        trueX1 + DisplayUtil.dip2px(objectBig / 2), trueY1 + DisplayUtil.dip2px(objectBig / 2)));
...
//                Log.e("我看到底是谁", "+++++++++++++++++1");
            ...
            if (nowClickAtmanRelation != null && otherRelation.getGroup() != nowClickAtmanRelation.getGroup()) {
                for (int g = 0; g < nowClickAtmanRelation.getParentGroups().size(); g++) {
                    if (nowClickAtmanRelation.getParentGroups().get(g).getGroup() == otherRelation.getGroup()) {
                        ...
                        TextView tv_item_zheng_release = relativeLayout.findViewById(R.id.tv_item_zheng_release);//连线中间的文字描述
                        ...
                        zhengjuBean.setGroupId3(nowClickAtmanRelation.getParentGroups().get(g).getRelation() + "");
                        zhengjuBean.setPicCount(nowClickAtmanRelation.getParentGroups().get(g).getPictureCount());
                        tv_item_zheng_release.setText(nowClickAtmanRelation.getParentGroups().get(g).getRelation());
                        ...
                        zhengjuLayout.height = DisplayUtil.dip2px(222);
                        zhengjuLayout.width = DisplayUtil.dip2px(222);
                        zhengjuLayout.leftMargin = zheng_x_center - DisplayUtil.dip2px(12.5f);
                        zhengjuLayout.topMargin = zheng_y_center - DisplayUtil.dip2px(12.5f);
                        ...
                        if (woqu.getGroup() == otherLinePdList.get(e).getGroup()) {
                            if (clickNode == -1) {
                                ...
                                if (woqu.getGroup() == clickNode) {
                                    ...
                                    RelativeLayout relativeLayout = (RelativeLayout) inflate(getContext(), R.layout.item_zhengju_release, null);
                                    TextView text_zhengjCount = relativeLayout.findViewById(R.id.text_zhengjCount);
                                    TextView tv_item_zheng_release = relativeLayout.findViewById(R.id.tv_item_zheng_release);
                                    ImageView zhengju_image = relativeLayout.findViewById(R.id.zhengju_image);
                                    ZhengjuBean zhengjuBean = new ZhengjuBean();
                                    zhengjuBean.setGroupId1(otherRelation.getGroup() + "");
                                    zhengjuBean.setGroupId2(woqu.getGroup() + "");
                                    zhengjuBean.setGroupId3(woqu.getRelation() + "");
                                    zhengjuBean.setPicCount(woqu.getPictureCount());
                                    tv_item_zheng_release.setText(woqu.getRelation());
                                    zhengju_image.setTag(zhengjuBean);
                                    zhengju_image.setOnClickListener(onClickListener);
    ...
    public void trueZheng(AtmanRelation findPairBean) {
        for (int n = 0; n < clickList.size(); n++) {
            if (findPairBean.getParentGroups().get(0).getGroup() == clickList.get(n).getGroup()) {//说明找到父类了。那么也确定了2个点。把证据加上去
                ...
                int trueXX1 = zheng_x_center - DisplayUtil.dip2px(12.5f);
                int trueYY1 = zheng_y_center - DisplayUtil.dip2px(12.5f);

                RectPoint leoPoint = new RectPoint(trueXX1 - DisplayUtil.dip2px(12.5f), trueYY1 - DisplayUtil.dip2px(12.5f),
                        trueXX1 + DisplayUtil.dip2px(12.5f), trueYY1 + DisplayUtil.dip2px(12.5f));
                ...
                if (findPairBean.getParentGroups().get(0).getPictureCount() > 0) {//证据数量大于0  才能加上去
                    if (checkHaveZhengjuRect(leoPoint)) {
                        ...
                        zhengjuBean.setGroupId3(findPairBean.getParentGroups().get(0).getRelation() + "");
                        zhengjuBean.setPicCount(findPairBean.getParentGroups().get(0).getPictureCount());
                        tv_item_zheng_release.setText(findPairBean.getParentGroups().get(0).getRelation());
                        zhengju_image.setTag(zhengjuBean);
                        zhengju_image.setOnClickListener(onClickListener);
    ...
    public void zhengjuNoG(int startX, int startY, int distanceX, int distanceY, AtmanRelation findPairBean, int numCount) {
        ...
        int trueXX1 = zheng_x_center - DisplayUtil.dip2px(15);
        int trueYY1 = zheng_y_center - DisplayUtil.dip2px(15);

        RectPoint leoPoint = new RectPoint(trueXX1 - DisplayUtil.dip2px(15), trueYY1 - DisplayUtil.dip2px(15),
                trueXX1 + DisplayUtil.dip2px(15), trueYY1 + DisplayUtil.dip2px(15));
        ...
        if (checkHaveZhengjuRect(leoPoint)) {
            ...
            zhengjuLayout.height = DisplayUtil.dip2px(222);
            zhengjuLayout.width = DisplayUtil.dip2px(222);
            zhengjuLayout.leftMargin = zheng_x_center - DisplayUtil.dip2px(15);
            zhengjuLayout.topMargin = zheng_y_center - DisplayUtil.dip2px(15);
            layouPoints.addView(relativeLayout, zhengjuLayout);
            rectsZhengj.add(leoPoint);
        }
    ...
    public void repetAdd(int x_center, int y_center, int circle, int currentJD, int relation, AtmanRelation itemBean) {
        int circleAdd = circle + DisplayUtil.dip2px(50);
        int X1 = 0;
        int Y1 = 0;
        if (currentJD != 0 && currentJD != 180 && currentJD != 360) {//别面有错误出现
