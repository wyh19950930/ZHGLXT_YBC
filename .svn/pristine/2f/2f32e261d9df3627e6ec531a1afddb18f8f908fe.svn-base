...
//        MapsInitializer.replaceURL(VEC_URL, "tdt-vecosm43")//添加天地图tdt-bdkb
...
//        mPresenter.getSmewmcyl("0006")
        ...
        if (AppCache.getInstance().zhqx == 1){
            tv_frag_ldgl_title.setText("北臧村镇房屋出租流动人口管理")//镇级流动人口管理
            ...
            tv_frag_ldgl_title.setText(AppCache.getInstance().xzqName+"房屋出租流动人口管理")//村级流动人口管理
...
//        mPresenter.getHylxtj()//查询首页行业类型统计
        ...
        tv_frag_ldgl_ckgd.setOnClickListener {
            //查看更多列表
            if (SingleOnClickUtil1.isFastClick()) {
                if (AppCache.getInstance().zhqx == 1) {//镇
                    val intent = Intent(this, CzlbActivity::class.java)
                    startActivity(intent)
                } else if (AppCache.getInstance().zhqx == 2) {//村
...
//        clearAllMarker()
    ...
    override fun returnJbqktj(msg: FirstTjBean) {
        ...
        rlv_frag_ldgl_hyfbqk.layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)//图例显示
